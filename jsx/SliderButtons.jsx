class SliderButtons extends React.Component {
  ...
  render() {
    return (
      <div>
        <button onClick={this.handleChange(-1)} disabled={this.state.sliderValue <= 0}>1 less ({this.state.sliderValue - 1})</button>
        <button onClick={this.handleChange(1)} disabled={this.state.sliderValue >= 100}>1 higher ({this.state.sliderValue + 1})</button>
      </div>
    )
  }
}